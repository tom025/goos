  ...
  class MainWindow
    describe SnipersTableModel do
      ...
      it 'has the correct number of columns' do
        model.column_count.should == Column.length
      end

      it 'sets the values of the columns' do
        listener.should_receive(:table_changed)

        model.add_sniper_snapshot(SniperSnapshot.joining('item-1234'))
      ...
      it 'notifies listeners when adding a sniper' do
        ...
        model.add_sniper_snapshot(joining_sniper_snapshot)
      ...
      it 'holds snipers in addition order' do
        model.add_sniper_snapshot(SniperSnapshot.joining('item-0'))
        model.add_sniper_snapshot(SniperSnapshot.joining('item-1'))
      ...
      it 'updates the correct row for the sniper' do
        ...
        model.add_sniper_snapshot(sniper_snapshot)
